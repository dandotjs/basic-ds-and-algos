...
class Tree {
  ...
  add(value) {
    ...
    if (!this.root) {
      ...
      // we want this to keep running until we tell it to stop
      // there's always going to be somewhere to add the value
      // which is why we use a while true loop
      while (true) {
        // if value to add is smaller than root value go left
        // else go right
        if (current.value > value) {
          // if we have a left value keep going through loop
          // with next node
          if (current.left) {
            current = current.left;
          } else {
            // if we dont have a left value we found our spot!
            current.left = new Node(value);
            break;
          }
        } else {
          // same logic as left but swapped with right
          if (current.right) {
